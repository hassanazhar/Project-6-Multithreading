...
Follow the submission instructions (based on the assignment description), 
and put your submission file in the submissions directory.

(We provide 3 dummy submissions already. Add your own submission file to them.)
 You might notice that the submissions directory structure for the dummy submissions is slightly different
 from the one requested from you in the PDF description.  That should not be a problem. 
 But do ensure that you don't include test case files or non-necessary .java files.

Then execute ./perform_grading.sh

WARNING: after running the grading script, your original files will be removed. 
Make a copy before grading.
 WARNING: do not change the structure of the provided methods at all, although your code 
might compile on your machine;
    it will cause a compilation error here as the tests do not 
expect your modified declarations.
...
If that zip file does not contain test_log.txt 
nor any xml file, it means you had submission structure violation.
If you have only one file named 
test_log.txt, it means you have had a compile error. You can find your error by inspecting test_log.txt.
...
Project6_someTestsFailure.zip: this submission has appropriate format and compiles. 
It passes some (not all) of the tests.
...
If you have trouble running this script on kamek, it  may be because you have modified .bash_profile and/or other .xxx files in your home directory.  Try replacing your modified files with the standard ones from a friend's directory.
